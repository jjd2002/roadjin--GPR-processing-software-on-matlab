function [gather,tout,srcx,srcz,recx,recz] = ...
    TM_model2d(ep,mu,sig,xprop,zprop,srcloc,recloc,srcpulse,t,npml,...
    outstep,plotopt,movieopt)
%
% TM_model2d.m : This is a 2-D, TM-mode, FDTD modeling program for
%                reflection ground-penetrating radar, implementing the
%                method of James Inrving and Rosemary Knight (2006,
%                Computers and Geosciences, 32 1247–1258).
%
% Features:
% - convolutional PML absorbing boundaries (Roden and Gedney, 2000)
% - calculations in PML boundary regions have been separated from main
%   modeling region for speed 
% - second-order accurate time derivatives, fourth-order-accurate spatial
%   derivatives (O(2,4)) 
%
% Syntax:  [gather,tout,srcx,srcz,recx,recz] = TM_model2d(ep,mu,sig,xprop,..
%                      zprop,srcloc,recloc,srcpulse,t,npml,outstep,plotopt) 
%
% where 
% gather      = output data cube (common source gathers are stacked along
%               the third index) 
% tout        = output time vector corresponding to first index in gather
%               matrix (s) 
% srcx        = vector containing actual source x locations after
%               discretization (m) 
% srcz        = vector containing actual source z locations after
%               discretization (m) 
% recx        = vector containing actual receiver x locations after
%               discretization (m) 
% recz        = vector containing actual receiver z locations after
%               discretization (m) 
% ep,mu,sig   = electrical property matrices  (rows = x, columns = z;
%               need odd # of rows and columns; already padded for PML
%               boundaries; ep and mu are relative to free space; sig units
%               are S/m)  
% xprop,zprop = position vectors corresponding to electrical property
%               matrices (m) 
...
% srcpulse    = source pulse vector (length must be equal to the number
%               of iterations) 
% t           = time vector corresponding to srcpulse (used for determining
%               time step and number of iterations) (s) 
% npml        = number of PML absorbing boundary cells
% outstep     = write a sample to the output matrix every so many
%               iterations (default=1) 
% plotopt     = plot Ey wavefield during simulation?  
%               (vector = [{0=no, 1=yes}, {output every # of iterations},
%               {colorbar threshold}]) (default = [1 50 0.05])
% movieopt    = Make an AVI video of the simulation when plotopt(1) = 1.
%               Structure, with fields: ans (1=YES, 0=N0), fname (AVI file
%               name), cpr (compression, 1="Cinepak", 0="None"), fps
%               (frames per second), q (quality, range 0-100). Defaults are
%               {0, 'work\Ey_wavefield.avi', 'Cinepak', 2, 80}.
%
% Notes:
% - all matrices in this code need to be transposed before plotting, as the
%   first index always refers to the horizontal direction, and the second
%   index to the vertical direction 
% - locations in space are referred to using x and z- indices in electric
%   and magnetic field matrices are referred to using (i,j)  [i=horizontal,
%   j=vertical] 
% - indices in electrical property matrices are referred to using (k,l)
%   [k=horizontal,l=vertical] 
%
% Written by James Irving, July 2005
%
% Modified by A. Tzanis (2008) to include movie (AVI file) generation and
% graphical progress reporting (i.e. a smart waitbar).
...
if size(mu)~=size(ep) | size(sig)~=size(ep);  %#ok<OR2>
    disp('TM_MODEL2D > ep, mu, and sig matrices must be the same size');
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return; 
end
if [length(xprop),length(zprop)]~=size(ep);  %#ok<BDSCA>
    disp('TM_MODEL2D > xprop and zprop are inconsistent with ep, mu, and sig'); 
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return; 
end
if mod(size(ep,1),2)~=1 || mod(size(ep,2),2)~=1; 
    disp('TM_MODEL2D > ep, mu, and sig must have an odd # of rows and columns'); 
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return; 
end
if size(srcloc,2)~=2 || size(recloc,2)~=2; 
    disp('TM_MODEL2D > srcloc and recloc matrices must have 2 columns');
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return;
end
if max(srcloc(:,1))>max(xprop) || min(srcloc(:,1))<min(xprop) || max(srcloc(:,2))>max(zprop)...
        || min(srcloc(:,2))<min(zprop); 
    disp('TM_MODEL2D > source vector out of range of modeling grid');
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return;
end 
if max(recloc(:,1))>max(xprop) || min(recloc(:,1))<min(xprop) || max(recloc(:,2))>max(zprop)...
        || min(recloc(:,2))<min(zprop);
    disp('TM_MODEL2D > receiver vector out of range of modeling grid');
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return;
end
if length(srcpulse)~=length(t); 
    disp('TM_MODEL2D > srcpulse and t vectors must have same # of points');
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return;
end
if npml>=length(xprop)/2 || npml>=length(zprop)/2; 
    disp('TM_MODEL2D > too many PML boundary layers for grid');
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return;
end
if length(plotopt)~=3; 
    disp('TM_MODEL2D > plotopt must be a 3 component vector');
    gather=[]; tout=[]; srcx=[]; srcz=[]; recx=[]; recz=[];
    return;
end

% Set up graphical display if plotopt(1) = 1
if plotopt(1)==1;
    Eyfig = findobj('tag','fdtdEyfigure');
    if ~isempty(Eyfig),
        figure(Eyfig);
        clf;
    else
        scrsz  = get(0,'screensize');
        fpos = round([700*scrsz(3)/1680 525*scrsz(4)/1050 ...
            560*scrsz(3)/1680 420*scrsz(4)/1050]);
        Eyfig = figure('name','FDTD','tag','fdtdEyfigure',...
            'menubar','none', 'numbertitle','off',...
            'position',fpos);
    end
end
%Initialize AVI file if movie is requested
if movieopt.ans == 1,
    Eymov = avifile(movieopt.fname,'compression',movieopt.cpr,...
        'fps', movieopt.fps, 'quality',movieopt.q);
end
% ------------------------------------------------------------------------------------------------
...
disp('TM_MODEL2D > Determining update coefficients for simulation region...')
...
disp('TM_MODEL2D > Beginning FDTD simulation...')
% initialize gather matrix where data will be stored
gather = zeros(fix((numit-1)/outstep)+1,nrec,nsrc);
% If plotting not wanted, create a wait bar to show progress
if plotopt(1) ~= 1,
    wbh = waitbar(0,'Running the simulation. Please be patient!');
    totalwork = nsrc*numit;
end
...
%                 disp(['Source ',num2str(s),'/',num2str(nsrc),', Iteration ',...
%                     num2str(it),'/',num2str(numit),...
%                     ':  t = ',num2str(t(it)*1e9),' ns'])
                figure(Eyfig); 
                imagesc(xEy,zEy,Ey'); 
                axis image
                title(['Source ',num2str(s),'/',num2str(nsrc),', Iteration ',...
                    num2str(it),'/',num2str(numit),...
                ...
                if movieopt.ans == 1,
                    F = getframe(gcf);
                    Eymov = addframe(Eymov,F);
                end
            end
        else
        % report progress
            waitbar((it + (s-1)*numit)/totalwork,wbh); 
        end
    ...
    if plotopt(1) ~= 1,
        if s == 1,
            tit2 = toc;
            ittime = (tit2 - tit1)/3600;
            totaltime = nsrc*ittime;
        end
        time2go   = totaltime - s*ittime;
        waitbar(s*numit/totalwork,wbh,...
            ['VERY patient! Source ', ...
            num2str(s),'/',num2str(nsrc),' with ',...
            num2str(time2go) ' hours to go!']);
    end
end
if plotopt(1) == 1,
    delete(Eyfig);
