function [dfk, fktype, vrange] = fk_filter(d, dt, dx)
%
% FK_FILTER : Design and apply an F-K filter. There are three options.
%             ** Zone pass / zone stop filtering:  This works like a band
%                pass or band reject filter. A polygonal zone is defined
%                graphically, by pointing and clicking at the desired
%                positions on the F-K spectrum map. "Zone pass" means that
%                ONLY the spectral contributions IN and ON the sides of the
%                zone will be retained. "Zone stop" means that ONLY the
%                spectral contributions OUTSIDE the zone will be retained.  
%             ** Velocity range pass / stop (velocity-fan filtering): The
%                [upper lower] negative, and [lower upper] positive
%                apparent velocity limits are defined by means of a dialog
%                box. "Velocity range pass " means that spectral
%                contributions within the [lower upper] apparent velocity
%                range will be retained. "Velocity range stop" means that
%                the spectral contributions outside the [lower, upper]
%                apparent velocity range will be retained.      
%             ** Up-dipping / down-dipping event separation: This stops an
%                entire F-K quadrant passing the other, thus rejecting
%                reflections with positive (up-dipping) or negative
%                (down-dipping) slopes.
%             => A necessary condition for the application of the F-K
%                filter is that the traces are equally spaced.
