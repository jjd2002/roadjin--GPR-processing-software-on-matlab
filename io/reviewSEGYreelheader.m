function HDR = reviewSEGYreelheader()
%
...
HDR.headertext= char(reshape(HDR.headertext, 40, 80));
...
clb(length(clb)+1) = cellstr(txt);
txt = ['Sampling interval (MATGPR): ' num2str(HDR.dt)];
clb(length(clb)+1) = cellstr(txt);
txt = ['Samples per trace         : ' num2str(HDR.hns)];
clb(length(clb)+1) = cellstr(txt);
txt = ['Number of traces          : ' num2str(HDR.ntrpr)];
clb(length(clb)+1) = cellstr(txt);
txt = ['Trace spacing (MATGPR)    : ' num2str(HDR.dx)];
clb(length(clb)+1) = cellstr(txt);
if (HDR.dx ==0 && HDR.dt==0) || round(HDR.dt * 1000) ~= HDR.hdt,  
    txt = 'NOTIFICATION              : THIS FILE WAS NOT MADE WITH MATGPR';
    clb(length(clb)+1) = cellstr( txt ); 
    txt = '                            OR SOME COMPLIANT PROGRAM!        ';
    clb(length(clb)+1) = cellstr( txt ); 
end
...
clb(length(clb)+1) = cellstr(txt);
for i = 1:40,
    ws = isspace(HDR.headertext(i,:));
    if sum(ws) == 80,                  % blank line
        continue
    else
        clb(length(clb)+1) = cellstr(HDR.headertext(i,:));
    end
end
